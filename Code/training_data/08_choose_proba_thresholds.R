...
eval_metrics <- read.csv("Data/training_evaluation_results/05c_indicator_thresholds.csv")[,-1]
eval_metrics <- eval_metrics %>% mutate(id = paste0(min_proba, min_distance, max_entropy))
eval_group_shares <- read.csv("Data/training_evaluation_results/05d_indicator_thresholds_group_sizes.csv")[,-1]
...
##################################################
#### Validate with performance of small LSTM #####
##################################################

LSTM <- read.csv("Data/training_evaluation_results/06_LSTM_performances.csv")[,-1]
LSTM %>% filter(max_proba == best_thresholds$min_proba, 
                dist_second == best_thresholds$min_distance)
LSTM %>% arrange(-accuracy)
paste("Mean accuracy on small LSTM is", mean(LSTM$accuracy))
...
p <- plot_ly(x = as.numeric(colnames(plot_grid)), 
  ...
  layout(scene = list(xaxis = list(title = "P<sub>h</sub>"),
                      yaxis = list(title = "\u0394"),
...
# https://www.r-graph-gallery.com/3d-surface-plot.html
